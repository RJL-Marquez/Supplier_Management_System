...
function getAccountInfo(accountEntity) {
  ...
  const nativeAccountId = (homeTenantProfile == null ? void 0 : homeTenantProfile.nativeAccountId) || accountEntity.nativeAccountId;
  return {
    ...
    if (!autodetectedRegionName) {
      ...
      try {
        ...
        if (localIMDSVersionResponse.status === HTTP_SUCCESS) {
          autodetectedRegionName = (_a = localIMDSVersionResponse.body) == null ? void 0 : _a.location;
          if (autodetectedRegionName) {
        ...
        if (localIMDSVersionResponse.status === HTTP_BAD_REQUEST) {
          ...
          if (currentIMDSVersionResponse.status === HTTP_SUCCESS) {
            autodetectedRegionName = (_b = currentIMDSVersionResponse.body) == null ? void 0 : _b.location;
            if (autodetectedRegionName) {
              regionDiscoveryMetadata.region_source = RegionDiscoverySources.IMDS;
            }
          }
  ...
  getAccountsFilteredBy(accountFilter, correlationId) {
    ...
    allAccountKeys.forEach((cacheKey) => {
      ...
      const tenantProfileFilter = {
        ...
        nativeAccountId: accountFilter == null ? void 0 : accountFilter.nativeAccountId
      };
...
var ThrottlingUtils = class _ThrottlingUtils {
  ...
  static preProcess(cacheManager, thumbprint, correlationId) {
    ...
    if (value) {
      ...
      throw new ServerError(((_a = value.errorCodes) == null ? void 0 : _a.join(" ")) || "", correlationId, value.errorMessage, value.subError);
    }
  }
  /**
...
function getStandardAuthorizeRequestParameters(authOptions, request, logger, performanceClient) {
  ...
  addScopes(parameters, requestScopes, request.correlationId, true, (_b = authOptions.authority.options.OIDCOptions) == null ? void 0 : _b.defaultScopes);
  addResource(parameters, request.resource);
  addRedirectUri(parameters, request.redirectUri);
...
var RefreshTokenClient = class {
  ...
  async createTokenRequestBody(request) {
    ...
    addScopes(parameters, request.scopes, request.correlationId, true, (_b = this.config.authOptions.authority.options.OIDCOptions) == null ? void 0 : _b.defaultScopes);
...
var CryptoOps = class _CryptoOps {
  ...
  async getPublicKeyThumbprint(request) {
    ...
    const keyPair = await generateKeyPair(_CryptoOps.EXTRACTABLE, _CryptoOps.POP_KEY_USAGES, RSA_KEYGEN_ALGORITHM_OPTIONS);
...
var PlatformAuthInteractionClient = class extends BaseInteractionClient {
  ...
  async acquireTokensFromCache(nativeAccountId, request) {
    ...
    try {
      ...
      const idTokenClaims = AuthToken_exports.extractTokenClaims((idToken == null ? void 0 : idToken.secret) || "", base64Decode, this.correlationId);
  ...
  async generateAuthenticationResult(response, request, idTokenClaims, accountEntity, authority, reqTimestamp) {
    ...
    if (accountInfo.nativeAccountId !== response.account.id) {
      ...
      const tenantProfile = (_a = accountInfo.tenantProfiles) == null ? void 0 : _a.get(targetTenantId);
      if (tenantProfile) {
  ...
  async initializePlatformRequest(request) {
    ...
    const mergedClaims = RequestParameterBuilder_exports.buildMergedClaims(claims, (configClaims == null ? void 0 : configClaims.length) ? configClaims : void 0);
    const validatedRequest = {
      ...remainingProperties,
      claims: mergedClaims,
...
var NestedAppAuthController = class _NestedAppAuthController {
  ...
  acquireTokenRedirect(request) {
    throw NestedAppAuthError.createUnsupportedError(request == null ? void 0 : request.correlationId);
  }
  /**
  ...
  logoutRedirect(logoutRequest) {
    throw NestedAppAuthError.createUnsupportedError(logoutRequest == null ? void 0 : logoutRequest.correlationId);
  }
  logoutPopup(logoutRequest) {
    throw NestedAppAuthError.createUnsupportedError(logoutRequest == null ? void 0 : logoutRequest.correlationId);
  }
...
function getPerfMeasurementModule() {
  ...
  try {
    ...
    if (Number(perfEnabled) === 1) {
      return import("./BrowserPerformanceMeasurement-7BKX62UW.js");
    }
  } catch (e) {
  }
  return void 0;
